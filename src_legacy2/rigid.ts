...
export function make_wander(vs: Vec2, opts: RigidOptions) {
  ...
  function update(dt: number, dt0: number) {
    ...
    let desired_vel: Vec2 | undefined = wander_steer(matrix, v_wander, 10, 500, 20)
...
type Body = any
export type Behaviour = any

export const b_orbit_steer = (target: Vec2) => (_body: Body) => orbit_steer(_body.vs, target, _body.max_speed)
export const b_no_steer = (_body: Body) => Vec2.zero

export const b_wander_steer = (jitter: number, r: number, distance: number) => {
  let v_wander = Vec2.unit
  return (_body: Body) => wander_steer(_body.matrix, v_wander, jitter, r, distance)
}

export const b_separation_steer = (group: Array<Vec2>) =>
(_body: Body) => separation_steer(_body.vs, group, _body.max_speed)

export const b_avoid_circle_steer = (target: Circle, zero_angle: number) =>
(_body: Body) => avoid_circle_steer(_body.vs, target, _body.max_speed, zero_angle)

export const b_flee_steer = (target: Vec2, zero_angle: number) =>
(_body: Body) => flee_steer(_body.vs, target, _body.max_speed, zero_angle, 100)

export const b_arrive_steer = (target: Vec2) => 
(_body: Body) => arrive_steer(_body.vs, target, _body.max_speed, 100)
...
function wall_avoid_steer(position: Matrix, length: number, walls: Array<Line>) {
  ...
  let steering_force: Vec2 | undefined

  let closest_dist: number,
  closest_wall: Line,
  closest_point: Vec2
